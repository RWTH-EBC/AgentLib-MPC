...
class MyCasadiModelConfig(CasadiModelConfig):
    ...
    parameters: List[CasadiParameter] = [
        ...
        CasadiParameter(
            name="max_power_weight",
            value=1,
            unit="-",
            description="Weight for max power in objective function",
        ),
        CasadiParameter(
            name="previous_max_power",
            value=0,
            unit="W",
            description="Slack variable of temperature of zone",
        ),
    ...
    outputs: List[CasadiOutput] = [
        ...
        CasadiOutput(name="cooling_power", unit="K", description="cooling power"),
    ]
...
class MyCasadiModel(CasadiModel):
    ...
    def setup_system(self):
        # Define ode
        power = self.cp * self.mDot * (self.T_in - self.T)

        self.T.ode = (power + self.load) / self.C

        # Define ae
        self.T_out.alg = self.T  # math operation to get the symbolic variable
        self.cooling_power.alg = power

        # Constraints: List[(lower bound, function, upper bound)]
        self.constraints = [
            # soft constraints
            (0, self.T + self.T_slack, self.T_upper),
            (-float("inf"), -power - self.max_power, 0),
            (0, self.max_power - self.previous_max_power, float("inf")),
        ]
        ...
        objective = sum(
            [
                ...
                self.max_power_weight * self.max_power,
            ]
        )
    ...
    "modules": [
        {"module_id": "Ag1Com", "type": "local_broadcast"},
        {
            ...
            "parameters": [
                {"name": "s_T", "value": 3},
                {"name": "r_mDot", "value": 1},
                {"name": "previous_max_power", "value": 0},
                {"name": "max_power_weight", "value": 1e-3},
            ],
            "inputs": [
            ...
            "outputs": [{"name": "T_out"}],
            "global_opt_vars": [{"name": "max_power", "value": 0, "lb": 0, "ub": 1000}],
            "states": [
                {
                    "name": "T",
                    "value": 298.16,
