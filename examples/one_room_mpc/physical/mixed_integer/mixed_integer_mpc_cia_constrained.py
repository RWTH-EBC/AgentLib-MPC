    ...
    "modules": [
        {"module_id": "Ag1Com", "type": "local_broadcast"},
        {
            ...
            "optimization_backend": {
                ...
                "solver": {
                    "name": "ipopt",
                    # Add solver options if needed, e.g., verbosity
                    # "options": {"ipopt": {"print_level": 0}}
                },
                "overwrite_result_file": True,
                "results_file": "results//mpc_cia_constrained.csv",  # Changed filename
                "save_results": True,
                # --- New CIA Constraint Configurations ---
                # "min_up_times": {"cooler_on": 1800},  # 10 minutes in seconds
                # "min_down_times": {"cooler_on": 900}, # 15 minutes in seconds
                # "initial_mode_states": {"cooler_on": False}, # Start with cooler OFF
                "prevent_mode_constraints": [
                    {
                        "mode_name": "cooler_on",
                        "threshold": 0.7,
                        "condition": "<=",
                        "interval_index": 0,  # Check first interval
                    }
                ]
                # --- End New CIA Constraint Configurations ---
            },
            "time_step": 300,  # 5 minutes
            "prediction_horizon": 10,  # Predict 50 minutes ahead
            "parameters": [
                {"name": "s_T", "value": 3},
...
def run_example(with_plots=True, log_level=logging.INFO, until=10000):
    ...
    if with_plots:
        ...
        fig.suptitle("CIA with Constraints")  # Updated title
        plot_mpc(
...
if __name__ == "__main__":
    # Run for longer to see effect of min up/down times
    run_example(with_plots=True, until=10800)  # Run for 3 hours
