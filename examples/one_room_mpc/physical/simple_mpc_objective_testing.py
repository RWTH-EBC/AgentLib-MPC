...
class MyCasadiModelConfig(CasadiModelConfig):
    ...
    parameters: List[CasadiParameter] = [
        ...
        CasadiParameter(
            ...
            value=600,
            unit="s",
            description="Time at which the objective is changed"
        )
    ]
    outputs: List[CasadiOutput] = [
        CasadiOutput(name="T_out", unit="K", description="Temperature of zone"),
        CasadiOutput(name="switch_test", unit="-", description="Test variable to check the change of the objective")
    ]
...
class MyCasadiModel(CasadiModel):
    ...
    def setup_system(self):
        ...
        # Test extreme nesting for evaluate_expression (math stream)
        self.switch_test.alg = ca.if_else(
            self.time < self.switch.sym,
            ca.if_else(
                self.time < 300, 
                ca.if_else(self.time == 0, 1, 2), 
                3
            ), 
            ca.if_else(
                self.time < 1000, 
