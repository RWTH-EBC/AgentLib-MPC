...
def run_example(with_plots=True, log_level=logging.INFO, until=8000, days=1, step_size=900):
    # Change the working directly so that relative paths work
    os.chdir(os.path.abspath(os.path.dirname(__file__)))
    logging.basicConfig(level=log_level)

    #pinn_path = "pinns/Trainer/ml_model.json"
    try:
    ...
    mas = LocalMASAgency(
        agent_configs=agent_configs(ml_model_path=str(pinn_path)),
        env=ENV_CONFIG,
    )
    mas.run(until=until)
...
if __name__ == "__main__":
    days = 1
    step_size = 300
    run_example(with_plots=True, until=3600*12, days=days, step_size=step_size)
