...
class CasadiMLModel(CasadiModel):
    ...
    def _fixed_during_integration(
        ...
        exclude = [v.name for v in self.differentials + self.algebraics + self.outputs]
    ...
    def _make_integrator(self, ignore_algebraics: bool = False) -> ca.Function:
        ...
        if (not self.differentials) and (ignore_algebraics or not algebraic_outputs):
        ...
        if self.differentials and (ignore_algebraics or not algebraic_outputs):
            ode = {
                "x": x,
                "p": par,
                "ode": self.system,
        ...
        dae = {
            ...
            "p": par,
            "ode": self.system,
            "z": ca.vertcat(*[var.sym for var in algebraic_outputs]),
    ...
    def _make_unified_predict_function(
        ...
        if not ignore_algebraics:
            alg_dict = {var.name: var.sym for var in self.algebraics + self.outputs}
        else:
            alg_dict = {}
    ...
    def _is_auxiliary(self, var: CasadiState):
        ...
        if var.alg is not None or var.ode is not None:
            return False
